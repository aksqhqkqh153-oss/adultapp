...
$zip = "C:\Users\최성규\Downloads\adultapp_20260422_build_fix_deploy_ready.zip"
$branch = "main"
$commitMsg = "update: fix home feed build errors and deploy guard"
...
# =========================
# 1) 실행 중 프로세스 종료
# taskkill 미사용 -> 프로세스 없을 때 오류 방지
# =========================
Write-Host "1) 실행 중 프로세스 종료"
foreach ($name in @("python","pythonw","node","npm","uvicorn")) {
    Get-Process -Name $name -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
}

# =========================
# 2) ZIP 덮어쓰기 (temp 없이 바로)
# =========================
Write-Host "2) ZIP 덮어쓰기"
Expand-Archive -LiteralPath $zip -DestinationPath $project -Force

# =========================
# 3) Git 최신화 (ZIP 변경분 보존 후 동기화)
# 순서: ZIP 덮어쓰기 -> stash -> fetch/reset/clean -> stash pop
# =========================
Write-Host "3) Git 최신화 / 원격 동기화"
git add -A
...
# =========================
# 7) Cloudflare Pages 로그인 확인 + 수동 업로드
# 이미 연동된 상태 기준 / 세션 없을 때만 login
# =========================
