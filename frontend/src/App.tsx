...
const feedSeed: FeedItem[] = Array.from({ length: 160 }, (_, idx) => ({
  id: idx + 1,
...
const FeedPoster = memo(function FeedPoster({ item, onAsk, saved, onToggleSave }: { item: FeedItem; onAsk: (item: FeedItem) => void; saved: boolean; onToggleSave: (feedId: number) => void }) {
  return (
...
export default function App() {
  ...
  }, [globalKeyword, homeTab]);

  const renderedFeed = useMemo(() => visibleFeed.slice(0, feedVisibleCount), [visibleFeed, feedVisibleCount]);
  const hasMoreFeed = renderedFeed.length < visibleFeed.length;

  const loadMoreFeed = useCallback(() => {
    setFeedVisibleCount((prev) => Math.min(prev + 8, visibleFeed.length));
  }, [visibleFeed.length]);
  ...
  const handleMainScroll = useCallback((event: React.UIEvent<HTMLElement>) => {
    if (activeTab !== "홈" || homeTab !== "피드" || !hasMoreFeed) return;
    const target = event.currentTarget;
    const remaining = target.scrollHeight - target.scrollTop - target.clientHeight;
    if (remaining < 320) {
      loadMoreFeed();
    }
  }, [activeTab, homeTab, hasMoreFeed, loadMoreFeed]);
  ...
  return (
    <div className="mobile-app-shell">
      ...
      <main className="mobile-main" onScroll={handleMainScroll}>
            ...
            {homeTab === "피드" ? (
              <>
                ...
                <div className="feed-post-list compact-scroll-list">
                  {renderedFeed.map((item, idx) => {
                    const sponsoredItem = sponsoredFeedProducts[Math.floor(idx / 4) % sponsoredFeedProducts.length];
                    return (
                      <div key={`feed-wrap-${item.id}`} className="feed-render-block">
                        <FeedPoster item={item} onAsk={openAskFromFeed} saved={savedFeedIds.includes(item.id)} onToggleSave={toggleSavedFeed} />
                        {(idx + 1) % 4 === 0 ? (
                          <SponsoredFeedProductCard item={sponsoredItem} saved={savedProductIds.includes(sponsoredItem.id)} onToggleSave={toggleSavedProduct} />
                        ) : null}
                      </div>
                    );
                  })}
                  {hasMoreFeed ? <div className="feed-loading-indicator">피드를 더 불러오는 중...</div> : <div className="feed-end-indicator">최신 피드까지 모두 확인했습니다.</div>}
                </div>
              </>
            ) : homeTab === "상품" ? (
              <>
