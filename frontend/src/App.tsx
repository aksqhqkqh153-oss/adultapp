...
const adminModeTabs = ["계정관리", "운영현황", "계정권한", "문서", "지침/정책/약관/기준/운영방침", "성인인증", "승인", "정산", "DB관리", "신고", "채팅", "기타"] as const;
const adminLegalDocumentDefinitions = [
  ...
  { key: "privacy_policy_guideline", label: "개인정보처리방침 작성지침", required: "수집항목, 이용목적, 보관기간, 제3자 제공, 위탁, 파기 기준" },
  { key: "legal_terms_pack", label: "이용약관, 청소년보호정책, 전자상거래 약관", required: "회원 의무, 청소년 접근 차단, 주문/결제/배송/취소/환불 기준" },
  { key: "seller_review_terms", label: "판매자 이용약관 및 사업자 심사 기준", required: "사업자 전환, 입점 심사, 정산계좌, 금지상품, 권한 회수" },
  { key: "peer_trade_policy", label: "개인 간 일반거래 허용/금지 품목 정책", required: "허용 품목, 금지 품목, 개봉상품 제한, 신고/차단 기준" },
  { key: "adult_product_review_criteria", label: "성인용품 상품 검수 기준표", required: "카테고리별 허용/제한/금지, 이미지·문구 검수, 관리자 승인 기준" },
  { key: "ugc_moderation_sla", label: "UGC 신고/차단/숨김/제재 운영 SLA", required: "신고 접수, 임시 숨김, 처리기한, 제재 단계, 증거보존" },
] as const;
const adminOperationalPolicyKeys = ["privacy_policy_guideline", "legal_terms_pack", "seller_review_terms", "peer_trade_policy", "adult_product_review_criteria", "ugc_moderation_sla"] as const;
...
const createDefaultAdminDocumentDrafts = (): AdminLegalDocumentDrafts => adminLegalDocumentDefinitions.reduce((acc, item) => {
  const defaultPolicyDrafts: Partial<Record<AdminLegalDocumentKey, string>> = {
    ...
    privacy_policy_guideline: `# 개인정보처리방침 작성지침\n\n## 작성 원칙\n- 실제 수집하는 개인정보만 항목별로 기재합니다.\n- 성인인증, 결제, 배송, 채팅, 신고, 일반거래 기능별 수집 목적과 보관기간을 분리합니다.\n- CI/DI, 휴대폰번호, IP 등 식별 가능 정보는 필요한 경우에만 저장하고, 미성년 차단 목적 값은 HMAC 해시값 중심으로 관리합니다.\n\n## 필수 작성 항목\n1. 수집 항목: 이메일, 닉네임, 성인인증 결과값, 주문/배송 정보, 결제 상태, 신고/차단 로그, 업로드 이미지 등 실제 처리 항목\n2. 이용 목적: 회원관리, 성인확인, 주문/배송, 판매자 정산, 고객상담, 불법·유해행위 방지\n3. 보관기간: 회원 탈퇴 시 파기, 관계 법령 또는 분쟁 대응 목적 보관 항목은 별도 기간 명시\n4. 제3자 제공: PG사, 본인확인기관, 배송사, 판매자에게 제공되는 항목과 목적\n5. 처리위탁: 서버/호스팅, 문자/메일, 결제, 본인확인, 이미지 저장소 등 위탁사 확정 후 기재\n6. 파기 절차: 보관기간 만료 또는 목적 달성 시 지체 없이 삭제 또는 익명화\n\n## 운영 메모\n- 본 문서는 운영 전 법무/개인정보 담당 검토 후 확정합니다.`,
    legal_terms_pack: `# 이용약관, 청소년보호정책, 전자상거래 약관\n\n## 이용약관 기준\n- 회원가입, 로그인, 계정정지, 탈퇴, 금지행위, 책임 제한을 명시합니다.\n- 미성년자, 타인 명의 인증, 불법 상품 등록, 불법 촬영물·음란물 업로드, 사기 거래를 금지합니다.\n- 운영자는 신고 접수 또는 정책 위반 시 게시물 숨김, 채팅 제한, 상품 판매중지, 계정정지 조치를 할 수 있습니다.\n\n## 청소년보호정책 기준\n- 본 서비스는 성인 이용자 전용이며 회원가입 및 주요 서비스 접근 시 성인인증을 요구합니다.\n- 청소년으로 확인된 경우 회원 생성 없이 차단 로그만 최소 저장합니다.\n- 청소년보호책임자 성명, 연락처, 문의 경로를 운영 정보에 표시합니다.\n\n## 전자상거래 약관 기준\n- 주문, 결제, 배송, 취소, 환불, 교환, 반품, 품절, 배송 지연 기준을 구분합니다.\n- 직접판매 상품과 입점사업자 판매 상품의 책임 주체를 명확히 표시합니다.\n- 위생·밀봉 상품은 개봉 여부에 따라 환불 제한이 있을 수 있음을 사전에 안내합니다.`,
    seller_review_terms: `# 판매자 이용약관 및 사업자 심사 기준\n\n## 사업자 전환 조건\n- 사업자등록번호, 통신판매업 신고번호, 대표자명, 상호, 사업장 주소, 고객센터, 반품지, 정산계좌를 제출해야 합니다.\n- 관리자는 제출 정보의 형식, 누락, 금지 업종 여부, 고객 응대 가능 여부를 검토합니다.\n\n## 상품등록 기준\n- 등록 상품은 기본적으로 승인대기 상태로 저장하고 관리자 검수 후 노출합니다.\n- 상품명, 이미지, 설명에 미성년 연상 표현, 불법 촬영물, 노골적 성행위 묘사, 과장 효능 표현을 금지합니다.\n\n## 정산/권한 기준\n- 정산은 결제 취소·환불·분쟁 가능 기간을 고려해 보류금 정책을 둘 수 있습니다.\n- 반복 민원, 허위 상품, 배송 지연, 금지상품 등록 시 판매자 권한을 회수할 수 있습니다.`,
    peer_trade_policy: `# 개인 간 일반거래 허용/금지 품목 정책\n\n## 허용 방향\n- 일반 회원은 성인 인증 후 개인 소장품 중 안전하고 위생 리스크가 낮은 항목만 등록할 수 있습니다.\n- 키링, 장식성 액세서리, 미개봉 일반 소품 등 비위생·저위험 품목 중심으로 운영합니다.\n\n## 금지 방향\n- 개봉 또는 사용 이력이 있는 성인기구, 체액·위생 위험이 있는 상품, 의약품·의료기기 오인 상품, 불법 촬영물, 미성년자 연상 상품은 금지합니다.\n- 구매자와 판매자 간 외부 결제 유도, 사기, 개인정보 요구, 만남 강요를 금지합니다.\n\n## 운영 조치\n- 신고 상품은 임시 숨김 후 관리자 검토를 진행합니다.\n- 금지 품목 등록 시 상품 삭제, 거래 제한, 계정정지, 재가입 제한을 적용할 수 있습니다.`,
    adult_product_review_criteria: `# 성인용품 상품 검수 기준표\n\n## 공통 검수\n- 모든 상품은 관리자 승인 전까지 미노출 상태로 유지합니다.\n- 상품명, 썸네일, 상세 이미지, 설명, 카테고리, 가격, 재고, 판매자 정보를 검수합니다.\n\n## 허용 예시\n- 콘돔, 러브젤, 오일 등 위생용품은 법적 표시사항과 사용상 주의사항을 확인합니다.\n- 키링, 귀걸이, 반지 등 액세서리는 성인 노출 이미지 없이 등록 가능합니다.\n\n## 제한/반려 예시\n- 노골적 신체 노출, 성행위 장면, 미성년 연상 표현, 불법 촬영물, 폭력·비동의 암시 문구는 반려합니다.\n- 의료·치료 효능을 암시하는 표현은 근거가 없으면 삭제 또는 반려합니다.\n\n## 처리 기준\n- 1차 위반: 수정 요청 또는 반려\n- 반복 위반: 상품 등록 제한\n- 중대 위반: 판매자 권한 회수 및 신고 보존`,
    ugc_moderation_sla: `# UGC 신고/차단/숨김/제재 운영 SLA\n\n## 적용 범위\n- 피드, 쇼츠, 스토리, 소통 게시글, 댓글, 질문, 채팅, 프로필, 상품 후기 전체에 적용합니다.\n\n## 신고 처리 기준\n- 긴급 신고: 불법 촬영물, 미성년 관련, 협박, 개인정보 유출, 사기 의심은 우선 검토합니다.\n- 일반 신고: 욕설, 스팸, 부적절 이미지, 허위 정보, 커뮤니티 분쟁은 순차 검토합니다.\n\n## 기본 SLA\n- 긴급 신고: 접수 후 가능한 한 신속히 임시 숨김 및 관리자 검토\n- 일반 신고: 운영일 기준 검토 큐 처리\n- 반복 신고 계정: 자동 제한 또는 관리자 검토 큐 등록\n\n## 제재 단계\n1. 콘텐츠 숨김 또는 수정 요청\n2. 기능 제한 또는 채팅 제한\n3. 일시 정지\n4. 영구 정지 및 재가입 제한\n\n## 기록 보존\n- 신고자/피신고자 원문 개인정보 노출을 최소화하고, 증거 보존 목적의 로그만 권한 있는 관리자에게 제한 표시합니다.`,
  };
  acc[item.key] = defaultPolicyDrafts[item.key] ?? `# ${item.label}\n\n필수 내용: ${item.required}\n\n- `;
  return acc;
...
const launchAdminEmails = ["aksqhqkqh3@naver.com"] as const;
const normalizeAdminRoleValue = (role: unknown) => String(role ?? "").trim().toUpperCase();
const isAdminRoleValue = (role: unknown) => {
  const normalized = normalizeAdminRoleValue(role);
  return ["ADMIN", "1", "GRADE_1", "SUPER_ADMIN", "ROOT", "OWNER", "MASTER", "최고관리자"].includes(normalized);
};
const isLaunchAdminEmail = (email: unknown) => {
  const normalized = String(email ?? "").trim().toLowerCase();
  return launchAdminEmails.includes(normalized as (typeof launchAdminEmails)[number]);
};
...
function DesktopSplitShell() {
  ...
  const desktopIsAdmin = isAdminRoleValue(desktopCurrentRole);
      ...
      <div className="stack-gap">
        ...
        {normalizedAdminMode === "계정권한" ? (
          <div className="settings-grid settings-two-col admin-mode-dashboard-grid">
            ...
            <div className="legacy-box compact admin-mode-summary-card"><h3>지침/정책/약관/기준/운영방침</h3><p>심사 제출과 실제 운영에 사용할 운영 기준 문서를 목록별로 작성합니다.</p><button type="button" className="ghost-btn" onClick={() => setAdminModeTab("지침/정책/약관/기준/운영방침")}>운영방침 열기</button></div>
          </div>
        ) : null}
        {normalizedAdminMode === "문서" ? (
          <div className="stack-gap admin-document-editor-shell">
            ...
            {adminLegalDocumentDefinitions.filter((item) => !adminOperationalPolicyKeys.includes(item.key as (typeof adminOperationalPolicyKeys)[number])).map((item) => (
        ...
        {normalizedAdminMode === "지침/정책/약관/기준/운영방침" ? (
          <div className="stack-gap admin-document-editor-shell admin-policy-editor-shell">
            <div className="legacy-box compact admin-document-editor-head">
              <div className="split-row">
                <div><h3>지침/정책/약관/기준/운영방침</h3><p>심사 제출과 실제 운영 기준으로 사용할 문서를 항목별로 수정하고 저장합니다.</p></div>
                <button type="button" onClick={saveAdminDocuments}>전체 저장</button>
              </div>
              {documentSaveMessage ? <p className="muted-mini">{documentSaveMessage}</p> : null}
            </div>
            {adminLegalDocumentDefinitions.filter((item) => adminOperationalPolicyKeys.includes(item.key as (typeof adminOperationalPolicyKeys)[number])).map((item) => (
              <div key={item.key} className="legacy-box compact admin-document-editor-card admin-policy-editor-card">
                <div className="admin-document-editor-title-row">
                  <div><h3>{item.label}</h3><p>작성 기준: {item.required}</p></div>
                  <span className="desktop-placeholder-pill">{adminLegalDocuments?.items?.[item.key]?.version ?? "draft"}</span>
                </div>
                <textarea
                  className="admin-document-textarea admin-policy-textarea"
                  value={documentDrafts[item.key] ?? ""}
                  onChange={(event) => updateDocumentDraft(item.key, event.target.value)}
                  placeholder={`${item.label} 내용을 입력하세요. 작성 기준: ${item.required}`}
                />
              </div>
            ))}
          </div>
...
export default function App() {
  ...
  const isAdmin = isAdminRoleValue(currentUserRole) || isLaunchAdminEmail(authSummary?.email);
...
변경 전 닉네임을 사용시 취소를 누르고, 변경 후 닉네임을 사용시 선택시 네를 눌러주세요`);
  ...
  const applyLoggedInUser = async () => {
    const me = await getJson<AuthMeResponse>("/auth/me");
    setAuthSummary(me);
    const nextRole = isLaunchAdminEmail(me.email) ? "1" : String(me.grade ?? "GUEST").toUpperCase();
    setCurrentUserRole(nextRole);
    if (typeof window !== "undefined") {
      window.localStorage.setItem("adultapp_demo_role", nextRole);
    }
