...
$zip = "C:\Users\최성규\Downloads\adultapp_20260423_pc_left_right_menu_fix.zip"
$branch = "main"
$commitMsg = "update: fix pc left right pane menu routing"
...
function Reset-GitConflictState {
    param([string]$RepoPath)

    Set-Location $RepoPath

    $gitLock = Join-Path $RepoPath ".git\index.lock"
...
# 0) Git 충돌 흔적 정리
# =========================
Write-Host "0) Git 충돌 상태 정리"
Reset-GitConflictState -RepoPath $project
...
# =========================
# 6) Git 반영 + Push (Railway 자동배포 트리거)
# =========================
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Wrangler 로그인 필요 - 브라우저 로그인 진행"
...
Write-Host "참고: Railway 가 GitHub 자동배포 상태면 origin/$branch push 후 backend adultapp 서비스가 자동 반영됩니다."
