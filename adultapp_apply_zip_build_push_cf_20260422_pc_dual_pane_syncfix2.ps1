...
$zip = "C:\Users\icj24\Downloads\adultapp_20260422_pc_dual_pane_syncfix2.zip"
$branch = "main"
...
function Reset-GitConflictState {
    ...
    if (Test-Path (Join-Path $RepoPath ".git\index.lock")) {
        Remove-Item (Join-Path $RepoPath ".git\index.lock") -Force -ErrorAction SilentlyContinue
    }
...
Write-Host ""
Write-Host "완료: Git 충돌 정리 / ZIP 덮어쓰기 / Git 최신화 / 프론트 빌드 / backend/static 반영 / Git Push / Cloudflare Pages 업로드"
